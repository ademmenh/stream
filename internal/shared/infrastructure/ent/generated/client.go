...
import (
	...
	"go-starter/internal/shared/infrastructure/ent/generated/migrate"
...
func (c *Client) Intercept(interceptors ...Interceptor) {
	c.UserSchema.Intercept(interceptors...)
...
func (c *UserSchemaClient) MapCreateBulk(slice any, setFunc func(*UserSchemaCreate, int)) *UserSchemaCreateBulk {
	rv := reflect.ValueOf(slice)
	if rv.Kind() != reflect.Slice {
		return &UserSchemaCreateBulk{err: fmt.Errorf("calling to UserSchemaClient.MapCreateBulk with wrong type %T, need slice", slice)}
	}
	builders := make([]*UserSchemaCreate, rv.Len())
	for i := 0; i < rv.Len(); i++ {
		builders[i] = c.Create()
...
func (c *UserSchemaClient) UpdateOne(_m *UserSchema) *UserSchemaUpdateOne {
	mutation := newUserSchemaMutation(c.config, OpUpdateOne, withUserSchema(_m))
	return &UserSchemaUpdateOne{config: c.config, hooks: c.Hooks(), mutation: mutation}
}

// UpdateOneID returns an update builder for the given id.
func (c *UserSchemaClient) UpdateOneID(id uuid.UUID) *UserSchemaUpdateOne {
...
func (c *UserSchemaClient) DeleteOne(_m *UserSchema) *UserSchemaDeleteOne {
	return c.DeleteOneID(_m.ID)
}

// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *UserSchemaClient) DeleteOneID(id uuid.UUID) *UserSchemaDeleteOne {
...
func (c *UserSchemaClient) Get(ctx context.Context, id uuid.UUID) (*UserSchema, error) {
	return c.Query().Where(userschema.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *UserSchemaClient) GetX(ctx context.Context, id uuid.UUID) *UserSchema {
...
func (c *UserSchemaClient) mutate(ctx context.Context, m *UserSchemaMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("generated: unknown UserSchema mutation op: %q", m.Op())
	}
}
...
func (c *VideoClient) mutate(ctx context.Context, m *VideoMutation) (Value, error) {
	...
	default:
		return nil, fmt.Errorf("generated: unknown Video mutation op: %q", m.Op())
	}
}
...
type (
	hooks struct {
		UserSchema, Video []ent.Hook
	}
	inters struct {
		UserSchema, Video []ent.Interceptor
	}
)
